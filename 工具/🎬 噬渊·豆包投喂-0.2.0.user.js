// ==UserScript==
// @name         🎬 噬渊·豆包投喂(成品提示词 → 填入+配图+发送)
// @namespace    luanshi_qingshu_doubao
// @version      0.2.0
// @description  豆包/即梦视频生成页:故事板四字段套成品模板,逐段填入+(可选)多选配图+发送。v0.2:配图改多选直传、可载入本地源文件、选择框稳定弹出。
// @author       乱世情书 Project
...
const SEL = {
  ...
  fileInput: 'input[type="file"]',                    // 豆包上传用的文件 input(可能隐藏)
...
注意不要把摄像机录像放到里面，16:9`);
  ...
  async function fillSend(seg) {
    fill(build(seg));
    if (IMAGES[seg.num]) { await sleep(200); await attach(IMAGES[seg.num]); await sleep(1300); }
  ...
  const css = `
  #sy-panel{position:fixed;right:18px;bottom:18px;width:344px;max-height:74vh;display:flex;flex-direction:column;
  ...
  panel.innerHTML = `
    ...
    <div id="sy-body">
      <div class="sy-row">
        ...
        <button class="sy-btn g" id="sy-demo">内置20段</button>
      ...
      <div class="sy-row">
        <button class="sy-btn" id="sy-img">📷 上传配图(多选)</button>
        <button class="sy-btn g" id="sy-imgclr">清空配图</button>
        <span class="sy-mut" id="sy-imgcnt"></span>
      </div>
      <div class="sy-divider"></div>
      <div id="sy-list" style="display:flex;flex-direction:column;gap:8px"></div>
      <div class="sy-mut">配图按文件名开头数字配段号(04_xx.png→第4段),或文件名含段标题。配图/发送是 v0.2,失灵就 F12 对 CONFIG.SEL。</div>
    </div>`;
  document.body.appendChild(panel);

  const $ = s => panel.querySelector(s);
  function imgCount() { return Object.keys(IMAGES).length; }
  function renderList() {
    $('#sy-mode').value = MODE;
    $('#sy-cnt').textContent = SEGS.length ? `共${SEGS.length}段` : '';
    $('#sy-imgcnt').textContent = imgCount() ? `已配${imgCount()}张` : '';
    const list = $('#sy-list'); list.innerHTML = '';
    if (!SEGS.length) { list.innerHTML = '<div class="sy-mut">点「📁 载入.md」选你的源,或「内置20段」。</div>'; return; }
    SEGS.forEach(s => {
      ...
      row.innerHTML = `<div class="t"><span class="sy-no">第${s.num}段</span>《${s.title}》${chip}${hasImg}</div>
        <div class="sy-row">
  ...
  $('#sy-demo').onclick = () => { SOURCE = DEFAULT_SOURCE; store.set('src', SOURCE); $('#sy-srcname').textContent = '内置20段'; reparse(); toast('已载入内置20段'); };
  ...
  $('#sy-img').onclick = () => pickFiles({ accept: 'image/*', multiple: true }, files => {
    let n = 0;
    for (const f of files) {
      if (!/\.(png|jpe?g|webp|gif|bmp)$/i.test(f.name)) continue;
      const mm = f.name.match(/^\s*0*(\d+)/);
      let key = mm ? +mm[1] : null;
      if (key == null || !SEGS.find(s => s.num === key)) {
        const seg = SEGS.find(s => f.name.includes(s.title)); if (seg) key = seg.num;
      }
      if (key != null) { IMAGES[key] = f; n++; }
    }
    toast(n ? `已配 ${n} 张(命中段号)` : '没匹配上:文件名开头数字要对应段号'); renderList();
  });
  $('#sy-imgclr').onclick = () => { IMAGES = {}; renderList(); toast('已清空配图'); };
  ...
  reparse();
  console.log('[噬渊·豆包投喂] v0.2.0 已加载,解析出', SEGS.length, '段');
})();
