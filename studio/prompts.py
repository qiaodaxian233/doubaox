...
DEFAULT_PROMPT_TEMPLATES = [
    ...
    PromptTemplate(
        id="tpl-vid-005",
        category="视频",
        title="Seedance 2.0 全能参考(@素材指认版)",
        placeholders=["at_lines", "clip_mode", "story", "shots", "lighting"],
        content=(
            "{at_lines}\n"
            "电影级国漫风格动画CG、电影级动画质感,动态运镜、{clip_mode},16:9,8K。\n"
            "画面:{story}\n"
            "镜头:{shots}\n"
            "光影氛围:{lighting}\n"
            ...
            "\n\n"
            "# 用法备注(粘贴时删掉本行以下):\n"
            "# 进豆包/即梦 Seedance 2.0「全能参考」入口,按 @图N 顺序上传素材\n"
            "# (惯例:图1=分镜线稿,之后依次角色卡,最后场景图)。\n"
            "# {at_lines} 形如: @图1 参考分镜构图与镜头顺序节奏,@图2 锁定男主角色(五官/发型/服饰一致),@图3 参考场景(按本段地点与光线)。\n"
            "# {clip_mode} 二选一: 10秒电影预告片(可快切蒙太奇) / 10秒一镜到底(舒缓推进,不要快切)。\n"
            "# 角色细节匹配度拉 80-100%;电影级选 P2.0 Pro;角色卡须为国漫风,不能传写实真人脸。"
        ),
    ),
